...
class LegacyNotificationAdapter {
  static final LegacyNotificationAdapter _instance =
  ...
  Future<List<RealNotificationModel>> getRealNotifications(
      String userId) async {
    EnhancedLogger.log(
        '🔄 [ADAPTER] Adaptando getRealNotifications para: $userId');

    try {
      return await _repository.getNotifications(userId);
    } catch (e) {
      EnhancedLogger.log(
          '❌ [ADAPTER] Erro no adaptador getRealNotifications: $e');
      return [];
    }
  }

  /// Adaptador para stream de notificações reais
  Stream<List<RealNotificationModel>> getRealNotificationsStream(
      String userId) {
  ...
  Stream<List<RealNotificationModel>> getMatchesNotificationStream(
      String userId) {
    EnhancedLogger.log(
        '🎯 [ADAPTER] Adaptando stream do MatchesController para: $userId');
    return _unifiedInterface.getUnifiedNotificationStream(userId);
  }

  /// Adaptador para busca simples de notificações
  Future<List<RealNotificationModel>> getSimpleNotifications(
      String userId) async {
  ...
  Future<void> forceNotificationSync(String userId) async {
    EnhancedLogger.log(
        '🚀 [ADAPTER] Adaptando força de sincronização para: $userId');
  ...
  Future<void> invalidateNotificationCache(String userId) async {
    EnhancedLogger.log(
        '🗑️ [ADAPTER] Adaptando invalidação de cache para: $userId');
  ...
  Future<void> resolveNotificationConflicts(String userId) async {
    EnhancedLogger.log(
        '⚡ [ADAPTER] Adaptando resolução de conflitos para: $userId');
  ...
  Future<bool> validateNotificationConsistency(String userId) async {
    EnhancedLogger.log(
        '🔍 [ADAPTER] Adaptando validação de consistência para: $userId');
  ...
  Future<void> migrateLegacySystem(String userId) async {
    ...
    try {
      ...
      if (isConsistent) {
        EnhancedLogger.log(
            '✅ [ADAPTER] Migração concluída com sucesso para: $userId');
      } else {
        EnhancedLogger.log(
            '⚠️ [ADAPTER] Migração com inconsistências para: $userId');
        await _unifiedInterface.resolveConflicts(userId);
      }
    } catch (e) {
  ...
  void dispose(String userId) {
    EnhancedLogger.log(
        '🧹 [ADAPTER] Limpando recursos do adaptador para: $userId');
