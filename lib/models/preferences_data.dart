...
class PreferencesData {
    ...
    try {
      EnhancedLogger.info('Creating PreferencesData from Firestore',
          ...
          data: {
            ...
            'allowInteractionsType':
                data['allowInteractions']?.runtimeType.toString(),
          });
      ...
      final allowInteractions = DataSanitizer.sanitizeBoolean(
          sanitizedData['allowInteractions'], true);

      final updatedAt =
          _extractDateTimeNonNull(sanitizedData['updatedAt'], DateTime.now());

      final version = sanitizedData['version'] as String? ??
          sanitizedData['dataVersion'] as String? ??
          '2.0.0';

      final lastSanitizedAt =
          _extractDateTime(sanitizedData['lastSanitizedAt'], null);

      final sanitizationVersion =
          sanitizedData['sanitizationVersion'] as String?;
  ...
  Map<String, dynamic> toFirestore() {
    ...
    if (!DataSanitizer.validateSanitizedData(data)) {
      EnhancedLogger.error('Invalid data in toFirestore',
          tag: 'PREFERENCES_DATA', data: data);
  ...
  bool get wasSanitized =>
      lastSanitizedAt != null && sanitizationVersion != null;
  ...
  static DateTime _extractDateTimeNonNull(
