...
class ChatOutroFormatoComponent extends StatelessWidget {
  ...
  const ChatOutroFormatoComponent({super.key, required this.item, required this.showArrow, required this.usuarioModel});
      ...
      showArrow: showArrow, 
        child: Container(
          ...
          child: InkWell(
            ...
            child: item.fileExtension?.toLowerCase() == 'pdf' ? Row(
              children: [
                Image.asset('lib/assets/img/pdf.png', width: 20),
                const SizedBox(width: 6),
                Flexible(child: Text('${item.fileName}', style: const TextStyle(color: Colors.white, fontSize: 15))),
              ],
            ) : Row(
              children: [
                Flexible(child: Text('${item.fileName}', style: const TextStyle(color: Colors.white, fontSize: 15))),
                const SizedBox(width: 6),
                Image.asset('lib/assets/img/download.png', width: 17, color: Colors.white)
              ],
            ),
          ),
      ),
