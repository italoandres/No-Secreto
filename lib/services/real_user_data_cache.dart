...
class RealUserDataCache {
  ...
  Future<UserData> getUserData(String userId, {Duration? ttl}) async {
    try {
      ...
      EnhancedLogger.success('Dados do usuário carregados e cacheados: ${userData.getDisplayName()}');
      
      return userData;
    } catch (e, stackTrace) {
      EnhancedLogger.error('Erro ao buscar dados do usuário', error: e, stackTrace: stackTrace);
  ...
  Future<Map<String, UserData>> preloadUsers(List<String> userIds, {Duration? ttl}) async {
    ...
    EnhancedLogger.info('Pré-carregando ${usersToFetch.length} usuários do Firebase');
    ...
    EnhancedLogger.success('Pré-carregamento concluído: ${results.length} usuários');
    return results;
  }
  ...
  void cleanupExpiredEntries({Duration? ttl}) {
    ...
    if (expiredUsers.isNotEmpty) {
      EnhancedLogger.info('Removidos ${expiredUsers.length} usuários expirados do cache');
    }
  }
  ...
  Future<UserData> _fetchUserFromFirebase(String userId) async {
    try {
      ...
      EnhancedLogger.error('Erro ao buscar usuário no Firebase: $userId', error: e);
      return UserData.fallback(userId);
    }
  }
  ...
  void debugCacheContents() {
    ...
    for (final entry in _cache.entries) {
      ...
      EnhancedLogger.debug('${entry.key}: ${entry.value.getDisplayName()} (${age}min ago)');
    }
