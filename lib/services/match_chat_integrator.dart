...
class MatchChatIntegrator {
    ...
    try {
      ...
      if (existingChat != null) {
        EnhancedLogger.info('Chat já existe, não criando duplicado',
            tag: 'MATCH_CHAT_INTEGRATOR', data: {'chatId': chatId});
        return chatId;
      }
      ...
      EnhancedLogger.error(
          'Erro ao criar chat a partir de interesse aceito: $e',
          tag: 'MATCH_CHAT_INTEGRATOR');
      return null;
    }
  }
  ...
  static Future<void> _sendWelcomeMessage({
    ...
    try {
      ...
      if (isMutualMatch) {
        welcomeMessage =
            '🎉 Vocês têm um match! Ambos demonstraram interesse um no outro. '
  ...
  static Future<void> _updateGlobalUnreadCount(String userId) async {
    try {
      ...
      for (final chat in userChats) {
        final unreadCount =
            await MatchChatRepository.getUnreadCount(chat.id, userId);
