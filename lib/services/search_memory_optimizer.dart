...
class SearchMemoryOptimizer {
  ...
  static SearchMemoryOptimizer get instance =>
      _instance ??= SearchMemoryOptimizer._();

  SearchMemoryOptimizer._();

  /// Pool de objetos reutilizáveis
  final Queue<SpiritualProfileModel> _profilePool =
      Queue<SpiritualProfileModel>();
  ...
  SpiritualProfileModel _createOptimizedProfile(
      Map<String, dynamic> data, bool enableLazyLoading) {
  ...
  bool _profileMatchesFilters(
      SpiritualProfileModel profile, List<FilterPredicate> predicates) {
  ...
  Map<String, dynamic> getMemoryStats() {
    final poolEfficiency =
        _totalAllocations > 0 ? _poolHits / (_poolHits + _poolMisses) : 0.0;
    ...
    return {
      ...
      'cache': {
        ...
        'cacheUsagePercentage': maxProcessedCache > 0
            ? _processedProfiles.length / maxProcessedCache
            : 0.0,
      },
      'metrics': {
        ...
        'poolEfficiencyPercentage':
            '${(poolEfficiency * 100).toStringAsFixed(1)}%',
      },
      'configuration': {
        'maxPoolSize': maxPoolSize,
        'maxProcessedCache': maxProcessedCache,
...
class ProcessedProfile {
  ...
  bool get isExpired =>
      ...
      SearchMemoryOptimizer.cacheExpiration;
}
