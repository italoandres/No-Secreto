...
class ChatSystemManager {
  ...
  static Future<String> createChat(String userId1, String userId2) async {
    ...
    try {
      ...
      await _sendWelcomeMessage(chatId, user1Data?['nome'] ?? 'Usuário',
          user2Data?['nome'] ?? 'Usuário');
    ...
    try {
      ...
      if (!chatDoc.exists) {
        ...
        if (userIds.length == 2) {
          ...
          throw Exception(
              'Não foi possível extrair IDs dos usuários do chatId: $chatId');
        }
      } else {
  ...
  static Future<void> updateUnreadCounters(
    ...
    try {
      ...
      if (!chatDoc.exists) {
        print(
            '❌ [CHAT_SYSTEM_MANAGER] Chat não encontrado para atualizar contadores');
        return;
      }

      final chatData = ChatData.fromJson(chatDoc.data()!);
      final otherUserId =
          chatData.participants.firstWhere((id) => id != senderId);
  ...
  static Future<void> markMessagesAsRead(String chatId, String userId) async {
    ...
    try {
      ...
      print(
          '✅ [CHAT_SYSTEM_MANAGER] ${unreadMessages.docs.length} mensagens marcadas como lidas');
    } catch (e) {
  ...
  static Future<List<MessageData>> getChatMessages(String chatId,
      {int limit = 50}) async {
    try {
      ...
      print(
          '📋 [CHAT_SYSTEM_MANAGER] ${messages.length} mensagens carregadas do chat $chatId');
  ...
  static Future<Map<String, dynamic>?> _getUserData(String userId) async {
    try {
      ...
      print(
          '❌ [CHAT_SYSTEM_MANAGER] Erro ao buscar dados do usuário $userId: $e');
      return null;
    }
  }
  ...
  static Future<void> _sendWelcomeMessage(
      String chatId, String user1Name, String user2Name) async {
    try {
      final welcomeMessage = MessageData(
        ...
        senderName: 'Sistema',
        message:
            '🎉 Parabéns! Vocês têm um match mútuo! Que tal começarem uma conversa?',
      ...
      print(
          '❌ [CHAT_SYSTEM_MANAGER] Erro ao enviar mensagem de boas-vindas: $e');
    }
  }

  /// Atualiza metadados do chat
  static Future<void> _updateChatMetadata(
      String chatId, MessageData message) async {
    try {
  ...
  static Future<void> _sendMessageNotification(
    ...
    try {
      ...
      final recipientId =
          chatData.participants.firstWhere((id) => id != message.senderId);
      ...
      print(
          '❌ [CHAT_SYSTEM_MANAGER] Erro ao enviar notificação de mensagem: $e');
    }
  }
