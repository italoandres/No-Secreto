...
class UnifiedNotificationCache {
  static final UnifiedNotificationCache _instance =
  ...
  List<NotificationModel>? getCachedNotifications(String userId) {
    if (!_cache.containsKey(userId)) {
      EnhancedLogger.log(
          '📭 [CACHE] Nenhuma notificação em cache para: $userId');
      return null;
    }
    ...
    EnhancedLogger.log(
        '✅ [CACHE] Retornando ${notifications.length} notificações do cache para: $userId');
  ...
  void updateCache(String userId, List<NotificationModel> notifications) {
    ...
    EnhancedLogger.log(
        '🔄 [CACHE] Cache atualizado para $userId: $previousCount → ${notifications.length} notificações');
  ...
  void _notifyListeners(String userId, List<NotificationModel> notifications) {
    if (_controllers.containsKey(userId)) {
      EnhancedLogger.log(
          '📡 [CACHE] Notificando listeners para $userId: ${notifications.length} notificações');
  ...
  void forceUpdate(String userId, List<NotificationModel> notifications) {
    EnhancedLogger.log(
        '🚀 [CACHE] Forçando atualização do cache para: $userId');
