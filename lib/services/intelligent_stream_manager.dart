...
class IntelligentStreamManager {
  static final IntelligentStreamManager _instance =
  ...
  Stream<List<RealNotificationModel>> getIntelligentStream(String userId) {
    EnhancedLogger.log(
        '🧠 [INTELLIGENT] Obtendo stream inteligente para: $userId');
  ...
  void _setupIntelligentStream(String userId) {
    EnhancedLogger.log(
        '⚙️ [INTELLIGENT] Configurando stream inteligente para: $userId');
    ...
    _subscriptions[userId] = _repository.watchNotifications(userId).listen(
  ...
  void _sendCachedDataIfAvailable(String userId) {
    ...
    if (cachedData != null) {
      ...
      EnhancedLogger.log(
          '⚡ [INTELLIGENT] Dados do cache enviados: ${cachedData.length} notificações');
    }
  }

  /// Processa atualização do stream
  void _handleStreamUpdate(
      String userId, List<RealNotificationModel> notifications) {
    EnhancedLogger.log(
        '📨 [INTELLIGENT] Atualização recebida: ${notifications.length} notificações');
    ...
    if (_hasSignificantChanges(cachedData, notifications)) {
      ...
      EnhancedLogger.log(
          '📊 [INTELLIGENT] Nenhuma mudança significativa detectada');
    }
  ...
  Future<void> _performIntelligentInvalidation(String userId) async {
    EnhancedLogger.log(
        '🧠 [INTELLIGENT] Executando invalidação inteligente para: $userId');

    try {
      // Verifica se o usuário ainda está ativo
      if (!_isUserActive(userId)) {
        EnhancedLogger.log(
            '😴 [INTELLIGENT] Usuário inativo, pulando invalidação');
        return;
      }
  ...
  Future<void> _attemptAutoRecovery(String userId) async {
    EnhancedLogger.log(
        '🔧 [INTELLIGENT] Tentando recuperação automática para: $userId');
