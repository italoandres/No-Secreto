...
class ConflictResolver {
  ...
  Future<bool> detectConflict(
    ...
    try {
      ...
      // Compara as fontes
      final hasConflict =
          _compareDataSources(cachedData, freshData, notifications);

      if (hasConflict) {
        EnhancedLogger.log('⚠️ [CONFLICT] Conflito detectado para: $userId');
        await _handleDetectedConflict(
            userId, cachedData, freshData, notifications);
      } else {
        EnhancedLogger.log(
            '✅ [CONFLICT] Nenhum conflito detectado para: $userId');
      }
  ...
  bool _compareDataSources(
    ...
    if (maxCount - minCount > 2) {
      EnhancedLogger.log(
          '⚠️ [CONFLICT] Diferença significativa nas quantidades: $counts');
      return true;
    }
  ...
  Future<void> _handleDetectedConflict(
    ...
    // Resolve conflito
    final resolution =
        await _resolveConflict(userId, cached, fresh, current, strategy);
  ...
  List<RealNotificationModel> _resolveByMerging(
    ...
    EnhancedLogger.log(
        '🔀 [CONFLICT] Mesclagem concluída: ${result.length} notificações');
    return result;
  }

  /// Resolve forçando nova busca
  Future<List<RealNotificationModel>> _resolveByForceRefresh(
      String userId) async {
  ...
  List<RealNotificationModel> _resolveByUserChoice(
    ...
    EnhancedLogger.log(
        '👤 [CONFLICT] Resolução por escolha do usuário (usando merge temporariamente)');
  ...
  Future<void> _applyResolution(
      String userId, ConflictResolution resolution) async {
    ...
    try {
      ...
      EnhancedLogger.log(
          '✅ [CONFLICT] Resolução aplicada: ${resolution.resolvedNotifications.length} notificações');
    } catch (e) {
  ...
  void _storeResolutionHistory(String userId, ConflictResolution resolution) {
    ...
    EnhancedLogger.log(
        '📚 [CONFLICT] Histórico atualizado: ${history.length} resoluções');
  }
  ...
  Future<List<RealNotificationModel>> resolveInconsistencies(
    ...
    EnhancedLogger.log(
        '🔧 [CONFLICT] Resolvendo inconsistências entre 2 fontes');
  ...
  Future<void> forceConsistency(String userId) async {
    ...
    try {
      ...
      EnhancedLogger.log(
          '✅ [CONFLICT] Consistência forçada: ${notifications.length} notificações');
    } catch (e) {
  ...
  void setupAutomaticConflictDetection(String userId) {
    EnhancedLogger.log(
        '🤖 [CONFLICT] Configurando detecção automática para: $userId');
  ...
  Future<bool> validateConsistency(String userId) async {
    ...
    try {
      ...
      if (!isConsistent) {
        EnhancedLogger.log(
            '⚠️ [CONFLICT] Inconsistência detectada, resolvendo...');
