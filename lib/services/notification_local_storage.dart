...
class NotificationLocalStorage {
  static final NotificationLocalStorage _instance =
  ...
  Future<bool> saveNotifications(
    ...
    try {
      ...
      EnhancedLogger.log(
          '💾 [LOCAL_STORAGE] Salvando ${notifications.length} notificações para: $userId');
  ...
  Future<List<RealNotificationModel>> loadNotifications(String userId) async {
    try {
      ...
      if (_memoryCache.containsKey(userId)) {
        EnhancedLogger.log(
            '🧠 [LOCAL_STORAGE] Carregando do cache em memória: ${_memoryCache[userId]!.length} notificações');
      ...
      if (jsonString == null) {
        EnhancedLogger.log(
            '📭 [LOCAL_STORAGE] Nenhuma notificação encontrada para: $userId');
        return [];
      }
      ...
      final notifications =
          jsonData.map((json) => RealNotificationModel.fromJson(json)).toList();

      // Atualiza cache em memória
      _memoryCache[userId] = List.from(notifications);

      EnhancedLogger.log(
          '📱 [LOCAL_STORAGE] Carregadas ${notifications.length} notificações do armazenamento');
  ...
  Future<bool> addNotification(
      String userId, RealNotificationModel notification) async {
    try {
      ...
      if (exists) {
        EnhancedLogger.log(
            '⚠️ [LOCAL_STORAGE] Notificação já existe: ${notification.id}');
        return true;
      }
  ...
  Future<bool> updateNotification(
    ...
    try {
      ...
      final index =
          currentNotifications.indexWhere((n) => n.id == notification.id);
      if (index != -1) {
  ...
  Future<bool> clearNotifications(String userId) async {
    try {
      ...
      EnhancedLogger.log(
          '🧹 [LOCAL_STORAGE] Notificações limpas para: $userId');
      return true;
    } catch (e) {
  ...
  Future<void> updateSyncStatus(String userId, SyncData syncData) async {
    try {
      ...
      EnhancedLogger.log(
          '🔄 [LOCAL_STORAGE] Status de sincronização atualizado: ${syncData.status}');
    } catch (e) {
      EnhancedLogger.log(
          '❌ [LOCAL_STORAGE] Erro ao atualizar status de sincronização: $e');
    }
  }
  ...
  Future<bool> hasLocalData(String userId) async {
    try {
      ...
      EnhancedLogger.log(
          '❌ [LOCAL_STORAGE] Erro ao verificar dados locais: $e');
      return false;
    }
  }
  ...
  Future<int> getStorageSize(String userId) async {
    try {
      ...
      EnhancedLogger.log(
          '❌ [LOCAL_STORAGE] Erro ao obter tamanho do armazenamento: $e');
      return 0;
    }
  }
  ...
  Future<bool> forceSync(String userId) async {
    try {
      EnhancedLogger.log(
          '🔄 [LOCAL_STORAGE] Forçando sincronização para: $userId');
  ...
  Future<List<RealNotificationModel>> _recoverFromBackup(String userId) async {
    try {
      ...
      EnhancedLogger.log(
          '✅ [LOCAL_STORAGE] Recuperadas ${notifications.length} notificações do backup');
      return notifications;
    } catch (e) {
  ...
  Future<void> _loadSyncStatus() async {
    try {
      final keys =
          _prefs!.getKeys().where((key) => key.startsWith(_syncPrefix));
      ...
      EnhancedLogger.log(
          '📊 [LOCAL_STORAGE] Status de sincronização carregado para ${_syncStatus.length} usuários');
    } catch (e) {
      EnhancedLogger.log(
          '⚠️ [LOCAL_STORAGE] Erro ao carregar status de sincronização: $e');
    }
  }
    ...
    _autoSaveTimers[userId] = Timer(Duration(seconds: 30), () async {
      try {
        if (_memoryCache.containsKey(userId)) {
          ...
          EnhancedLogger.log(
              '💾 [LOCAL_STORAGE] Auto-save executado para: $userId');
        }
      } catch (e) {
        ...
        if (backupString != null) {
          try {
            ...
            if (now.difference(timestamp).inDays > 7) {
              await _prefs!.remove(key);
              EnhancedLogger.log(
                  '🗑️ [LOCAL_STORAGE] Backup antigo removido: $key');
            }
          } catch (e) {
