...
class NotificationPerformanceOptimizer {
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      EnhancedLogger.success(
          '✅ [PERFORMANCE_OPTIMIZER] Otimizador inicializado');
    } catch (e) {
      EnhancedLogger.error('❌ [PERFORMANCE_OPTIMIZER] Erro ao inicializar',
          error: e);
    }
  }
  ...
  void _setupPeriodicOptimization() {
    ...
    EnhancedLogger.info(
        '⏰ [PERFORMANCE_OPTIMIZER] Otimização periódica configurada');
  }
  ...
  void _performPeriodicOptimization() async {
    try {
      EnhancedLogger.info(
          '🔧 [PERFORMANCE_OPTIMIZER] Executando otimização periódica');
      ...
      EnhancedLogger.success(
          '✅ [PERFORMANCE_OPTIMIZER] Otimização periódica concluída');
    } catch (e) {
      EnhancedLogger.error(
          '❌ [PERFORMANCE_OPTIMIZER] Erro na otimização periódica',
          error: e);
    }
  }
  ...
  Future<void> _optimizeCaches() async {
    try {
      ...
      final repoStats =
          EnhancedRealInterestsRepository.instance.getStatistics();
      final cacheSize = repoStats['cacheSize'] as int? ?? 0;

      if (cacheSize > _maxCacheSize) {
        EnhancedLogger.info(
            '    🧹 Cache do repository muito grande, limpando');
  ...
  Future<void> _optimizeBatchProcessing() async {
    try {
      ...
      EnhancedLogger.info(
          '    📊 Tamanho de lote otimizado: $optimalBatchSize');
      _recordMetric('optimalBatchSize', optimalBatchSize);

      EnhancedLogger.info('    ✅ Processamento em lote otimizado');
    } catch (e) {
      EnhancedLogger.error('    ❌ Erro ao otimizar processamento em lote',
          error: e);
    }
  }
  ...
  Future<void> _optimizeMemoryUsage() async {
    try {
      ...
      final optimizationTime =
          afterOptimization.difference(beforeOptimization).inMilliseconds;
  ...
  void _updatePerformanceMetrics() {
    try {
      ...
      final pipelineStats =
          FixedNotificationPipeline.instance.getPipelineStatistics();
      final converterStats =
          RobustNotificationConverter.instance.getConversionStatistics();
      final cacheStats = OfflineNotificationCache.instance.getCacheStatistics();

      // Registrar métricas
      _recordMetric(
          'pipelineProcessed', pipelineStats['processingLogSize'] ?? 0);
      _recordMetric(
          'conversionsTotal', converterStats['totalConversions'] ?? 0);
      _recordMetric('cacheSize', cacheStats['cachedNotifications'] ?? 0);

      EnhancedLogger.info('📊 [PERFORMANCE_OPTIMIZER] Métricas atualizadas');
    } catch (e) {
      EnhancedLogger.error(
          '❌ [PERFORMANCE_OPTIMIZER] Erro ao atualizar métricas',
          error: e);
    }
  }
  ...
  Future<List<dynamic>> optimizeUserProcessing(String userId) async {
    ...
    try {
      EnhancedLogger.info(
          '⚡ [PERFORMANCE_OPTIMIZER] Otimizando processamento para usuário',
          data: {'userId': userId});
      ...
      if (cachedNotifications.isNotEmpty) {
        ...
        EnhancedLogger.success(
            '💾 [PERFORMANCE_OPTIMIZER] Cache hit para usuário',
            data: {
              'userId': userId,
      ...
      EnhancedLogger.success(
          '⚡ [PERFORMANCE_OPTIMIZER] Processamento otimizado concluído',
          data: {
            'userId': userId,
      ...
      EnhancedLogger.error(
          '❌ [PERFORMANCE_OPTIMIZER] Erro no processamento otimizado',
          error: e,
  ...
  Future<Map<String, List<dynamic>>> optimizeBatchProcessing(
      List<String> userIds) async {
    final stopwatch = Stopwatch()..start();

    try {
      EnhancedLogger.info(
          '📦 [PERFORMANCE_OPTIMIZER] Iniciando processamento em lote otimizado',
      ...
      for (int i = 0; i < batches.length; i++) {
        ...
        final futures =
            batch.map((userId) => optimizeUserProcessing(userId)).toList();
      ...
      EnhancedLogger.success(
          '✅ [PERFORMANCE_OPTIMIZER] Processamento em lote concluído',
          data: {
      ...
      EnhancedLogger.error(
          '❌ [PERFORMANCE_OPTIMIZER] Erro no processamento em lote',
          error: e,
          data: {
            'userCount': userIds.length,
            'time': stopwatch.elapsedMilliseconds
  ...
  int _calculateOptimalBatchSize(double avgProcessingTime) {
    ...
    if (avgProcessingTime < 100) {
      return min(
          _batchSize * 2, 100); // Aumentar lote se processamento é rápido
  ...
  void _cleanupOldOptimizationLogs() {
    ...
    EnhancedLogger.info(
        '🧹 [PERFORMANCE_OPTIMIZER] Logs de otimização antigos limpos',
  ...
  Future<void> forceFullOptimization() async {
    try {
      EnhancedLogger.info(
          '⚡ [PERFORMANCE_OPTIMIZER] Forçando otimização completa');
      ...
      EnhancedLogger.success(
          '✅ [PERFORMANCE_OPTIMIZER] Otimização completa concluída');
    } catch (e) {
      EnhancedLogger.error(
          '❌ [PERFORMANCE_OPTIMIZER] Erro na otimização completa',
          error: e);
    }
  }
  ...
  List<String> getOptimizationRecommendations() {
    ...
    if (processingTimes.isNotEmpty) {
      ...
      if (avgTime > 2000) {
        recommendations.add(
            'Tempo de processamento alto (${avgTime.round()}ms). Considere otimizar queries ou aumentar cache.');
      } else if (avgTime < 100) {
        recommendations.add(
            'Processamento muito rápido (${avgTime.round()}ms). Considere aumentar tamanho do lote.');
      }
    }
    ...
    if (cacheHits.isNotEmpty && cacheMisses.isNotEmpty) {
      ...
      if (hitRate < 50) {
        recommendations.add(
            'Taxa de cache hit baixa ($hitRate%). Considere aumentar TTL do cache ou pré-carregar dados.');
      } else if (hitRate > 90) {
        recommendations.add(
            'Excelente taxa de cache hit ($hitRate%). Sistema bem otimizado.');
      }
    }
    ...
    if (batchSizes.isNotEmpty) {
      final avgBatchSize =
          batchSizes.reduce((a, b) => a + b) / batchSizes.length;

      if (avgBatchSize > 100) {
        recommendations.add(
            'Lotes muito grandes (${avgBatchSize.round()}). Considere reduzir para melhorar responsividade.');
      } else if (avgBatchSize < 10) {
        recommendations.add(
            'Lotes muito pequenos (${avgBatchSize.round()}). Considere aumentar para melhor throughput.');
      }
    }

    if (recommendations.isEmpty) {
      recommendations.add(
          'Sistema funcionando dentro dos parâmetros normais. Nenhuma otimização crítica necessária.');
    }
  ...
  void dispose() {
    try {
      ...
      EnhancedLogger.error(
          '❌ [PERFORMANCE_OPTIMIZER] Erro ao finalizar otimizador',
          error: e);
    }
  }
}
