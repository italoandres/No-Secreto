...
class NotificationMonitoringSystem {
  ...
  void initialize() {
    ...
    try {
      ...
      EnhancedLogger.success(
          '✅ [MONITORING_SYSTEM] Sistema inicializado com sucesso');
    } catch (e) {
  ...
  void _setupRealTimeMonitoring() {
    ...
    EnhancedLogger.info(
        '📊 [MONITORING_SYSTEM] Monitoramento em tempo real configurado',
  ...
  void _setupAutomaticAlerts() {
    ...
    EnhancedLogger.info(
        '🚨 [MONITORING_SYSTEM] Sistema de alertas configurado');
  }
  ...
  void _performRealTimeCheck() {
    try {
      ...
      EnhancedLogger.info(
          '📊 [MONITORING_SYSTEM] Verificação em tempo real concluída',
          data: {
      ...
      EnhancedLogger.error(
          '❌ [MONITORING_SYSTEM] Erro na verificação em tempo real',
          error: e);
    }
  }
  ...
  Map<String, dynamic> _collectSystemMetrics() {
    ...
    try {
      ...
      metrics['diagnostic'] =
          AdvancedDiagnosticSystem.instance.getSystemStatistics();

      // Métricas do sistema de recuperação
      metrics['recovery'] =
          ErrorRecoverySystem.instance.getRecoveryStatistics();
  ...
  void _sendAlert(String issue) {
    try {
      ...
      EnhancedLogger.warning('🚨 [MONITORING_SYSTEM] Alerta gerado',
          tag: 'NOTIFICATION_MONITORING_SYSTEM', data: alert);
    } catch (e) {
  ...
  void _checkForAlerts() {
    try {
      ...
      EnhancedLogger.error(
          '❌ [MONITORING_SYSTEM] Erro na verificação de alertas',
          error: e);
    }
  }
  ...
  void _performQuickDiagnostic() {
    try {
      ...
      if (conversionRate < _conversionThreshold) {
        _sendAlert(
            'Taxa de conversão baixa: ${conversionRate.toStringAsFixed(2)}%');
      }
    } catch (e) {
  ...
  Map<String, dynamic> getMonitoringStatistics() {
    return {
      ...
      'lastAlertTime':
          _alertHistory.isNotEmpty ? _alertHistory.last['timestamp'] : null,
      'timestamp': DateTime.now().toIso8601String(),
    };
  }
