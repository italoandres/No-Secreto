...
class VitrineShareService {
  ...
  Future<String> generatePublicLink(String userId) async {
    try {
      EnhancedLogger.info('Generating public link',
          tag: 'VITRINE_SHARE', data: {'userId': userId});
      ...
      EnhancedLogger.success('Public link generated',
          tag: 'VITRINE_SHARE', data: {'userId': userId, 'link': publicLink});
  ...
  Future<String> generateShareableContent(String userId) async {
    try {
      ...
      final profileDoc =
          await _firestore.collection('spiritual_profiles').doc(userId).get();
      ...
      shareText +=
          'Olá! Sou $userName e gostaria de compartilhar meu propósito com você.\n\n';
  ...
  Future<void> copyLinkToClipboard(String link) async {
    try {
      ...
      EnhancedLogger.info('Link copied to clipboard',
          tag: 'VITRINE_SHARE', data: {'link': link});
    } catch (e, stackTrace) {
  ...
  Future<bool> validateAccessToken(String userId, String token) async {
    try {
      final doc = await _firestore.collection('share_links').doc(userId).get();
  ...
  Future<void> _saveShareLinkInfo(
      String userId, String accessToken, DateTime expirationTime) async {
    await _firestore.collection('share_links').doc(userId).set({
      'userId': userId,
  ...
  Future<void> _trackShareAction(String userId, ShareType type) async {
    try {
      await _firestore.collection('share_analytics').add({
        'userId': userId,
