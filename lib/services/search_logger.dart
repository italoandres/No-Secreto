...
class SearchLogger {
  ...
  void logSearchEvent({
    ...
    EnhancedLogger.debug('Search event', 
      tag: 'SEARCH_LOGGER',
      data: {
        'operationId': operationId,
        'eventType': eventType,
  ...
  Map<String, dynamic> getSearchMetrics({Duration? timeWindow}) {
    ...
    final successfulOperations = relevantEntries.where((e) => e.error == null).length;
    final failedOperations = totalOperations - successfulOperations;
    final successRate = totalOperations > 0 ? successfulOperations / totalOperations : 0.0;
    ...
    final avgExecutionTime = executionTimes.isNotEmpty ? 
        executionTimes.reduce((a, b) => a + b) / executionTimes.length : 0.0;
    
    executionTimes.sort();
    final medianExecutionTime = executionTimes.isNotEmpty ? 
        executionTimes[executionTimes.length ~/ 2] : 0;
    
    // Métricas de cache
    final cacheHits = relevantEntries.where((e) => e.fromCache == true).length;
    final cacheHitRate = totalOperations > 0 ? cacheHits / totalOperations : 0.0;
    ...
    operationGroups.forEach((operation, entries) {
      ...
      operationStats[operation] = {
        ...
        'avgExecutionTime': opTimes.isNotEmpty ? 
            opTimes.reduce((a, b) => a + b) / opTimes.length : 0.0,
      };
    });
    ...
    for (final entry in relevantEntries) {
      if (entry.strategy != null) {
        strategyFrequency[entry.strategy!] = (strategyFrequency[entry.strategy!] ?? 0) + 1;
      }
    }
    ...
    return {
      ...
      'performance': {
        ...
        'minExecutionTime': executionTimes.isNotEmpty ? executionTimes.first : 0,
      ...
      'topQueries': topQueries.take(10).map((e) => {
        'query': e.key,
        'count': e.value,
      }).toList(),
  ...
  void _detectRepetitiveQueries(SearchLogEntry entry) {
    ...
    if (sameQueryCount >= 5) {
      final pattern = SearchPattern(
        ...
        description: 'Query "${entry.query}" executada ${sameQueryCount}x na última hora',
    ...
    if (entry.filters!.minAge != null || entry.filters!.maxAge != null) {
      ...
      if (ageFilterCount >= 10) {
        final pattern = SearchPattern(
          ...
          description: 'Filtros de idade usados em ${ageFilterCount} buscas recentes',
  ...
  void _detectPerformancePatterns(SearchLogEntry entry) {
    ...
    if (entry.executionTime! > avgTime * 2 && entry.executionTime! > 1000) {
      final pattern = SearchPattern(
        type: 'slow_operation',
        description: 'Operação ${entry.operationName} executou em ${entry.executionTime}ms (média: ${avgTime.round()}ms)',
        frequency: 1,
        detectedAt: DateTime.now(),
        data: {
          'operationName': entry.operationName,
  ...
  SearchLogEntry? _findEntryById(String operationId) {
    try {
      return _searchHistory.firstWhere((entry) => entry.operationId == operationId);
    } catch (e) {
      return null;
    }
  }
  ...
  void clearHistory() {
    ...
    EnhancedLogger.info('Search history cleared', 
      tag: 'SEARCH_LOGGER'
    );
  }
