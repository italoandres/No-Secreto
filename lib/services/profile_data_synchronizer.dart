...
class ProfileDataSynchronizer {
  ...
  static Future<void> syncUserData(String userId) async {
    await ErrorHandler.safeExecute(
      () async {
        ...
        if (user == null) {
          EnhancedLogger.warning('User not found for sync', tag: 'SYNC', data: {'userId': userId});
          return;
        }
        ...
        if (profile == null) {
          EnhancedLogger.info('No spiritual profile found, skipping sync', tag: 'SYNC', data: {'userId': userId});
          return;
        }
        ...
        if (!syncNeeded.needsSync) {
          EnhancedLogger.debug('No sync needed', tag: 'SYNC', data: {'userId': userId});
          return;
        }
        ...
        EnhancedLogger.success('User data synchronized successfully', tag: 'SYNC', data: {
          'userId': userId,
    ...
    await ErrorHandler.safeExecute(
      () async {
        ...
        if (user == null || profile == null) {
          EnhancedLogger.warning('Cannot resolve conflicts - missing data', tag: 'SYNC', data: {
            'userId': userId,
            'hasUser': user != null,
        ...
        final userLastUpdate = user.lastSyncAt ?? DateTime.fromMillisecondsSinceEpoch(0);
        final profileLastUpdate = profile.lastSyncAt ?? DateTime.fromMillisecondsSinceEpoch(0);
        
        final useUserAsSource = userLastUpdate.isAfter(profileLastUpdate);
        
        EnhancedLogger.info('Resolving conflicts using source', tag: 'SYNC', data: {
          'userId': userId,
        ...
        EnhancedLogger.success('Data conflicts resolved', tag: 'SYNC', data: {'userId': userId});
      },
  ...
  static Future<void> updateUsername(String userId, String newUsername) async {
    await ErrorHandler.safeExecute(
      () async {
        EnhancedLogger.sync('Updating username across collections', userId, data: {
        ...
        EnhancedLogger.success('Username updated in both collections', tag: 'SYNC', data: {
          'userId': userId,
          'username': newUsername,
        });
      },
  ...
  static Future<void> updateProfileImage(String userId, String? imageUrl) async {
    await ErrorHandler.safeExecute(
      () async {
        EnhancedLogger.sync('Updating profile image across collections', userId, data: {
        ...
        EnhancedLogger.success('Profile image updated in both collections', tag: 'SYNC', data: {
          'userId': userId,
        });
      },
  ...
  static Future<void> updateDisplayName(String userId, String newName) async {
    await ErrorHandler.safeExecute(
      () async {
        EnhancedLogger.sync('Updating display name across collections', userId, data: {
          'newName': newName,
        });
        ...
        EnhancedLogger.success('Display name updated in both collections', tag: 'SYNC', data: {
          'userId': userId,
          'name': newName,
        });
      },
  ...
  static SyncNeeded _needsSync(UsuarioModel user, SpiritualProfileModel profile) {
  ...
  static Future<void> _syncFromUserToProfile(UsuarioModel user, SpiritualProfileModel profile) async {
    final updates = <String, dynamic>{
  ...
  static Future<void> _syncFromProfileToUser(UsuarioModel user, SpiritualProfileModel profile) async {
    final updates = <String, dynamic>{
  ...
  static Future<SyncStatus> getSyncStatus(String userId) async {
    try {
      ...
      return SyncStatus(
        userId: userId,
        status: syncNeeded.needsSync ? SyncStatusType.conflicted : SyncStatusType.synced,
  ...
  static Future<void> batchSyncUsers({int limit = 50}) async {
    try {
      EnhancedLogger.info('Starting batch user synchronization', tag: 'SYNC', data: {'limit': limit});
      ...
      EnhancedLogger.success('Batch synchronization completed', tag: 'SYNC', data: {
        'total': usersQuery.docs.length,
        'synced': syncedCount,
        'errors': errorCount,
      });
    } catch (e) {
      EnhancedLogger.error('Batch synchronization failed', tag: 'SYNC', error: e);
    }
  }
}
