...
class ProfileCompletionDetector {
  ...
  static Future<ProfileCompletionStatus> getCompletionStatus(String userId) async {
    return await ErrorHandler.safeExecute<ProfileCompletionStatus>(
      () async {
        ...
        if (_statusCache.containsKey(userId)) {
          ...
          if (DateTime.now().difference(cachedStatus.completedAt ?? DateTime.now()).inSeconds < 30) {
            return cachedStatus;
          }
        }
  ...
  static Future<void> onProfileCompleted(String userId, VoidCallback callback) async {
  ...
  static void disposeAll() {
    ...
    EnhancedLogger.info('Disposed all completion detector resources', tag: _tag);
  }
}