...
class DataSanitizer {
  ...
  static Map<String, dynamic> sanitizePreferencesData(
      Map<String, dynamic> data) {
    final sanitized = <String, dynamic>{};
    final corrections = <String>[];

    EnhancedLogger.info('Starting preferences data sanitization',
        tag: _tag, data: {'originalFields': data.keys.toList()});
    ...
    for (final field in booleanFields) {
      if (data.containsKey(field)) {
        ...
        final sanitizedValue =
            sanitizeBoolean(originalValue, _getDefaultForField(field));
        ...
        if (originalValue != sanitizedValue) {
          ...
          EnhancedLogger.info('Field sanitized', tag: _tag, data: {
            'field': field,
    ...
    if (data.containsKey('completionTasks')) {
      ...
      if (originalTasks != sanitizedTasks) {
        ...
        EnhancedLogger.info('CompletionTasks sanitized', tag: _tag, data: {
    ...
    if (corrections.isNotEmpty) {
      ...
      EnhancedLogger.success('Data sanitization completed', tag: _tag, data: {
  ...
  static bool sanitizeBoolean(dynamic value, bool defaultValue) {
    ...
    if (value is Timestamp) {
      ...
      EnhancedLogger.info('Converting Timestamp to boolean', tag: _tag, data: {
  ...
  static Map<String, bool> sanitizeCompletionTasks(dynamic tasks) {
    ...
    if (tasks is! Map) {
      EnhancedLogger.warning('CompletionTasks is not a Map', tag: _tag, data: {
    ...
    try {
      ...
      for (final field in requiredBooleans) {
        if (!data.containsKey(field) || data[field] is! bool) {
          EnhancedLogger.error(
              'Validation failed: missing or invalid boolean field',
              tag: _tag,
              data: {
                'field': field,
                'value': data[field],
      ...
      if (data.containsKey('completionTasks')) {
        ...
        if (tasks is! Map<String, bool>) {
          EnhancedLogger.error(
              'Validation failed: completionTasks is not Map<String, bool>',
              tag: _tag,
              data: {
                'type': tasks.runtimeType.toString(),
                'value': tasks.toString(),
