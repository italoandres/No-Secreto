...
class RealInterestNotificationService {
  static final RealInterestNotificationService _instance =
      RealInterestNotificationService._internal();
  factory RealInterestNotificationService() => _instance;
  RealInterestNotificationService._internal();

  final RealInterestsRepository _interestsRepository =
  ...
  Future<List<RealNotification>> getRealInterestNotifications(
      String userId) async {
    try {
      EnhancedLogger.info(
      ...
      EnhancedLogger.info(
          '📊 [REAL_NOTIFICATIONS] Encontrados ${interests.length} interesses');
      ...
      final userIds =
          interests.map((interest) => interest.from).toSet().toList();
      EnhancedLogger.info(
          '👥 [REAL_NOTIFICATIONS] Buscando dados de ${userIds.length} usuários');
      ...
      final notifications = await _converter.convertInterestsToNotifications(
          interests, usersData);
      EnhancedLogger.info(
          '🔄 [REAL_NOTIFICATIONS] ${notifications.length} notificações convertidas');

      // 5. Agrupa notificações por usuário (evita spam)
      final groupedNotifications =
          _converter.groupNotificationsByUser(notifications);
      EnhancedLogger.success(
          '🎉 [REAL_NOTIFICATIONS] ${groupedNotifications.length} notificações REAIS encontradas');
      ...
      EnhancedLogger.error('❌ [REAL_NOTIFICATIONS] Erro ao buscar notificações',
  ...
  Stream<List<RealNotification>> subscribeToRealTimeUpdates(String userId) {
    try {
      EnhancedLogger.info(
          '🔄 [REAL_NOTIFICATIONS] Iniciando stream em tempo real para: $userId');

      _currentUserId = userId;

      return _interestsRepository
          .streamInterestsForUser(userId)
          .asyncMap((interests) async {
        EnhancedLogger.info(
            '📡 [REAL_NOTIFICATIONS] Stream atualizado: ${interests.length} interesses');
        ...
        EnhancedLogger.success(
            '🎉 [REAL_NOTIFICATIONS] Stream: ${groupedNotifications.length} notificações');
        return groupedNotifications;
      });
    } catch (e, stackTrace) {
      EnhancedLogger.error('❌ [REAL_NOTIFICATIONS] Erro no stream',
          error: e, stackTrace: stackTrace);
  ...
  Future<List<RealNotification>> refreshNotifications(String userId) async {
    EnhancedLogger.info(
        '🔄 [REAL_NOTIFICATIONS] Forçando atualização para: $userId');
  ...
  Future<List<RealNotification>> getRecentNotifications(String userId) async {
    try {
      EnhancedLogger.info(
          '⏰ [REAL_NOTIFICATIONS] Buscando notificações recentes para: $userId');

      final interests =
          await _interestsRepository.getRecentInterestsForUser(userId);

      if (interests.isEmpty) {
        EnhancedLogger.info(
            '📭 [REAL_NOTIFICATIONS] Nenhuma notificação recente');
        return [];
      }
      ...
      EnhancedLogger.success(
          '⏰ [REAL_NOTIFICATIONS] ${notifications.length} notificações recentes');
      return notifications;
    } catch (e, stackTrace) {
      EnhancedLogger.error(
          '❌ [REAL_NOTIFICATIONS] Erro ao buscar notificações recentes',
          error: e,
  ...
  Future<List<RealNotification>> _getFallbackNotifications(
      String userId) async {
    try {
      EnhancedLogger.warning(
          '🔄 [REAL_NOTIFICATIONS] Usando fallback para: $userId');
      ...
      for (final interest in interests) {
        try {
          ...
          final notification =
              _converter.convertInterestToNotification(interest, userData);
          notifications.add(notification);
        } catch (e) {
          EnhancedLogger.error(
              'Erro no fallback para interesse: ${interest.id}',
              error: e);
        }
      }

      EnhancedLogger.info(
          '🔄 [REAL_NOTIFICATIONS] Fallback: ${notifications.length} notificações');
      return notifications;
    } catch (e, stackTrace) {
      EnhancedLogger.error('❌ [REAL_NOTIFICATIONS] Erro no fallback',
          error: e, stackTrace: stackTrace);
      return [];
    }
  }
