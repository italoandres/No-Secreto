...
class ErrorRecoverySystem {
  ...
  void initialize() {
    ...
    try {
      ...
      EnhancedLogger.success(
          '✅ [ERROR_RECOVERY] Sistema inicializado com sucesso');
    } catch (e) {
  ...
  void _setupHealthMonitoring() {
    ...
    EnhancedLogger.info(
        '🏥 [ERROR_RECOVERY] Monitoramento de saúde configurado');
  }
  ...
  Future<void> recoverFromFailure() async {
    try {
      EnhancedLogger.info(
          '🔄 [ERROR_RECOVERY] Iniciando recuperação automática');
      ...
      try {
        ...
        EnhancedLogger.error(
            '❌ [ERROR_RECOVERY] Erro ao reinicializar JS Handler',
            error: e);
      }
      ...
      EnhancedLogger.success('✅ [ERROR_RECOVERY] Recuperação concluída', data: {
  ...
  Future<void> _restartCriticalComponents() async {
    try {
      ...
      EnhancedLogger.info(
          '🔄 [ERROR_RECOVERY] Componentes críticos reiniciados');
    } catch (e) {
      EnhancedLogger.error('❌ [ERROR_RECOVERY] Erro ao reiniciar componentes',
          error: e);
    }
  ...
  void logSystemState(Map<String, dynamic> state) {
    try {
      ...
      EnhancedLogger.info('📊 [ERROR_RECOVERY] Estado do sistema registrado',
          tag: 'SYSTEM_STATE', data: enhancedState);
    } catch (e) {
  ...
  List<RealNotification> getFallbackNotifications(String userId) {
    try {
      ...
      EnhancedLogger.info('💾 [ERROR_RECOVERY] Usando dados de fallback',
          data: {'userId': userId, 'fallbackCount': fallbackData.length});
  ...
  void saveFallbackNotifications(
      String userId, List<RealNotification> notifications) {
    try {
  ...
  void _performHealthCheck() {
    try {
      ...
      if (!isSystemHealthy) {
        ...
        if (lastAttempt == null ||
            now.difference(lastAttempt) > _recoveryInterval) {
          _lastRecoveryAttempts['system'] = now;

          EnhancedLogger.warning(
              '⚠️ [ERROR_RECOVERY] Sistema não saudável - iniciando recuperação');
          recoverFromFailure();
        }
      } else {
  ...
  void _cleanupOldRecoveryAttempts() {
    ...
    _lastRecoveryAttempts
        .removeWhere((key, timestamp) => timestamp.isBefore(cutoff));
  }
  ...
  Future<List<RealNotification>> recoverNotifications(String userId) async {
    try {
      EnhancedLogger.info(
          '🔄 [ERROR_RECOVERY] Recuperando notificações específicas',
          data: {'userId': userId});
      ...
      try {
        ...
        if (interests.isNotEmpty) {
          final converter = TempNotificationConverter.instance;
          final notifications =
              await converter.convertInteractionsToNotifications(interests, {});

          // Salva no fallback para próximas vezes
          saveFallbackNotifications(userId, notifications);

          EnhancedLogger.success(
              '✅ [ERROR_RECOVERY] Recuperação via busca direta',
      ...
      EnhancedLogger.error(
          '❌ [ERROR_RECOVERY] Erro na recuperação de notificações',
          error: e,
          data: {'userId': userId});
      return [];
    }
  ...
  List<RealNotification> _createEmergencyNotifications(String userId) {
    try {
      ...
      EnhancedLogger.error(
          '❌ [ERROR_RECOVERY] Erro ao criar notificações de emergência',
          error: e);
      return [];
    }
  }
