...
class UsernameManagementService {
  ...
  static Future<bool> isUsernameAvailable(String username) async {
    return await ErrorHandler.safeExecute(
          () async {
            ...
            EnhancedLogger.debug('Checking username availability',
                tag: 'USERNAME',
                data: {
                  'username': username,
                });
          ...
          () async {
            ...
            if (!DataValidator.isValidUsernameFormat(newUsername)) {
              ErrorHandler.showWarning(
                  'Username deve ter entre 3-30 caracteres, começar com letra/número e conter apenas letras, números, pontos e underscores.');
              return false;
            }
            ...
            final currentUsername =
                currentUserDoc.data()?['username'] as String?;

            // Atualizar histórico de usernames
            List<String> usernameHistory = [];
            if (currentUserDoc.exists &&
                currentUserDoc.data()?['usernameHistory'] != null) {
              usernameHistory =
                  List<String>.from(currentUserDoc.data()!['usernameHistory']);
            }
            ...
            if (currentUsername != null && currentUsername.isNotEmpty) {
              ...
              if (usernameHistory.length > 5) {
                usernameHistory =
                    usernameHistory.sublist(usernameHistory.length - 5);
              }
            }
            ...
            EnhancedLogger.success('Username updated successfully',
                tag: 'USERNAME',
                data: {
                  'userId': userId,
                  'newUsername': newUsername,
  ...
  static Future<List<String>> generateSuggestions(String baseName) async {
    return await ErrorHandler.safeExecute(
          () async {
            EnhancedLogger.debug('Generating username suggestions',
                tag: 'USERNAME',
                data: {
                  'baseName': baseName,
                });
            ...
            final suggestions =
                DataValidator.generateUsernameSuggestions(baseName);
            ...
            EnhancedLogger.debug('Username suggestions generated',
                tag: 'USERNAME',
                data: {
                  'baseName': baseName,
  ...
  static Future<bool> canChangeUsername(String userId) async {
    return await ErrorHandler.safeExecute(
          () async {
            ...
            final lastChange =
                userDoc.data()?['usernameChangedAt'] as Timestamp?;
            if (lastChange == null) {
  ...
  static Future<UsernameChangeInfo> getChangeInfo(String userId) async {
    return await ErrorHandler.safeExecute(
          () async {
            ...
            final daysUntilNextChange =
                canChange ? 0 : (30 - daysSinceLastChange);
  ...
  static Future<String?> reserveUsername(String username) async {
    return await ErrorHandler.safeExecute(
      () async {
        final reservationId =
            '${DateTime.now().millisecondsSinceEpoch}_${FirebaseAuth.instance.currentUser?.uid}';
  ...
  static Future<void> cleanExpiredReservations() async {
    await ErrorHandler.safeExecute(
      () async {
        ...
        EnhancedLogger.info('Cleaned expired username reservations',
            tag: 'USERNAME',
            data: {
