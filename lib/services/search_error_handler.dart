...
class SearchErrorHandler {
  ...
  static SearchErrorHandler get instance => _instance ??= SearchErrorHandler._();
  ...
  Duration _calculateRetryDelay(int attempt, SearchErrorType errorType) {
    ...
    var delay = Duration(
      milliseconds: (baseRetryDelay.inMilliseconds * (1 << (attempt - 1))).toInt()
    );
    ...
    // Aplicar jitter (±25%)
    final jitter = (delay.inMilliseconds * 0.25 * (DateTime.now().millisecond / 1000.0 - 0.5)).toInt();
  ...
  void _recordError(String operation, SearchErrorType errorType, Exception error, int attempt) {
  ...
  Map<String, dynamic> getErrorStats() {
    ...
    for (final errorType in SearchErrorType.values) {
      ...
      errorTypeStats[errorType.toString()] = {
        ...
        'minutesSinceLastOccurrence': lastTime != null ? 
          now.difference(lastTime).inMinutes : null,
      };
    }
  ...
  Map<String, int> _groupRecentErrorsByType(List<SearchErrorEvent> recentErrors) {
  ...
  List<Map<String, dynamic>> getRecentErrors({int limit = 20}) {
    return _errorHistory
        ...
        .map((event) => {
          ...
          'minutesAgo': DateTime.now().difference(event.timestamp).inMinutes,
        })
        .toList();
  }
  ...
  void clearErrorHistory() {
    ...
    EnhancedLogger.info('Error history cleared', 
      tag: 'SEARCH_ERROR_HANDLER'
    );
  }

  /// Verifica se um tipo de erro está ocorrendo frequentemente
  bool isErrorTypeFrequent(SearchErrorType errorType, {
