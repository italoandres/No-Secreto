...
class MessageReadStatusService {
  ...
  static Future<void> markAllMessagesAsRead({
    ...
    try {
      EnhancedLogger.info(
          'Marcando todas as mensagens como lidas para chat: $chatId',
          tag: 'MESSAGE_READ_STATUS');
      ...
      final unreadMessages = await _firestore
          ...
          .where('senderId',
              isNotEqualTo: userId) // Não marcar próprias mensagens
      ...
      EnhancedLogger.info(
          '${unreadMessages.docs.length} mensagens marcadas como lidas',
          tag: 'MESSAGE_READ_STATUS');
    } catch (e) {
  ...
  static Future<void> _updateUnreadCount(String chatId, String userId) async {
    try {
      ...
      EnhancedLogger.info(
          'Contador atualizado: $unreadCount mensagens não lidas',
          tag: 'MESSAGE_READ_STATUS');
    } catch (e) {
    ...
    try {
      ...
      return {
        ...
        'readPercentage': totalMessages > 0
            ? (readMessages / totalMessages * 100).round()
            : 0,
      };
    } catch (e) {
