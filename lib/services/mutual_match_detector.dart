...
class MutualMatchDetector {
  ...
  static Future<bool> checkMutualMatch(String userId1, String userId2) async {
    print(
        '🔍 [MUTUAL_MATCH_DETECTOR] Verificando match mútuo entre $userId1 e $userId2');
    ...
    try {
      ...
      print(
          '📊 [MUTUAL_MATCH_DETECTOR] Interesse de $userId1 para $userId2: ${interest1to2.docs.length} documentos');
      ...
      print(
          '📊 [MUTUAL_MATCH_DETECTOR] Interesse de $userId2 para $userId1: ${interest2to1.docs.length} documentos');

      final hasMutualMatch =
          interest1to2.docs.isNotEmpty && interest2to1.docs.isNotEmpty;
  ...
  static Future<void> createMutualMatchNotifications(
    ...
    try {
      ...
      print(
          '✅ [MUTUAL_MATCH_DETECTOR] Notificações de match mútuo criadas com sucesso');
    } catch (e) {
  ...
  static Future<String> triggerChatCreation(
      String userId1, String userId2) async {
  ...
  static Future<bool> isMatchAlreadyProcessed(
      String userId1, String userId2) async {
    try {
      final matchId = _generateChatId(userId1, userId2);
      final doc =
          await _firestore.collection('mutual_matches').doc(matchId).get();
      ...
      print(
          '❌ [MUTUAL_MATCH_DETECTOR] Erro ao verificar se match foi processado: $e');
      return false;
    }
  }

  /// Obtém todos os matches mútuos de um usuário
  static Future<List<Map<String, dynamic>>> getUserMutualMatches(
      String userId) async {
    ...
    try {
      ...
      print(
          '✅ [MUTUAL_MATCH_DETECTOR] ${matchList.length} matches mútuos encontrados');
  ...
  static Future<Map<String, dynamic>?> _getUserData(String userId) async {
    try {
      ...
      print(
          '❌ [MUTUAL_MATCH_DETECTOR] Erro ao buscar dados do usuário $userId: $e');
      return null;
    }
  }
  ...
  static Future<void> testMutualMatchDetector() async {
    ...
    try {
      ...
      final alreadyProcessed =
          await isMatchAlreadyProcessed(testUserId1, testUserId2);
