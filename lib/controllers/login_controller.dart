...
class LoginController {
  ...
  static validadeLogin() {
    ...
    debugPrint('Senha digitada: ${senha.isNotEmpty ? '[SENHA PREENCHIDA]' : '[SENHA VAZIA]'}');
  ...
  static Future<void> recuperarSenhaSemEmail() async {
    ...
    try {
      ...
      if (userEmail == null) {
        ...
        Get.defaultDialog(
          title: '❌ Erro',
          content: const Text('Usuário não está logado ou email não disponível.', textAlign: TextAlign.center),
          actions: [
            ElevatedButton(
              onPressed: () => Get.back(),
              child: const Text('Ok'),
  ...
  static Future<void> testarRecuperacaoSenha() async {
    ...
    try {
      ...
      await FirebaseAuth.instance.sendPasswordResetEmail(email: 'teste@exemplo.com');
    } catch (e) {
      ...
      if (e.toString().contains('configuration-not-found') || 
        ...
        debugPrint('❌ PROBLEMA: Templates de email não configurados no Firebase Console');
      } else if (e.toString().contains('user-not-found')) {
        debugPrint('✅ CONFIGURAÇÃO OK: Firebase consegue processar emails (usuário não encontrado é esperado)');
      } else {
  ...
  static Future<void> recuperarSenha() async {
    ...
    Get.defaultDialog(
      ...
      content: Column(
        children: [
          const Text('Digite abaixo seu E-mail de acesso que enviaremos um link para recuperação de sua senha', textAlign: TextAlign.center),
          const SizedBox(height: 12),
          TextField(
        ...
        SizedBox(
          ...
          child: TextButton(
            onPressed: () async {
              ...
              Get.rawSnackbar(
                message: '🔍 Testes executados! Verifique os logs no console.',
