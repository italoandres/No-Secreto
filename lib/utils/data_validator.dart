...
class DataValidator {
  ...
  static Map<String, dynamic> validateSpiritualProfile(
    ...
    try {
      ...
      for (final field in booleanFields) {
        if (validatedData[field] != null && validatedData[field] is! bool) {
          issues.add(
              '$field has incorrect type: ${validatedData[field].runtimeType}');
        ...
        if (tasks is Map<String, dynamic>) {
          ...
          for (final entry in tasks.entries) {
            if (entry.value is! bool) {
              issues.add(
                  'completionTasks.${entry.key} has incorrect type: ${entry.value.runtimeType}');
      ...
      if (issues.isNotEmpty) {
        EnhancedLogger.warning('Data validation issues found',
            tag: 'VALIDATION',
            data: {
              'issues': issues,
    ...
    try {
      ...
      if (validatedData['username'] != null) {
        ...
        validatedData['username'] =
            _sanitizeUsername(validatedData['username']);
      }
      ...
      if (issues.isNotEmpty) {
        EnhancedLogger.warning('User data validation issues found',
            tag: 'VALIDATION',
            data: {
              'issues': issues,
  ...
  static String _sanitizeString(String input) {
    ...
    String sanitized = input
        .replaceAll(
            RegExp(r'[\x00-\x1F\x7F]'), '') // Remove caracteres de controle
  ...
  static List<String> generateUsernameSuggestions(String name) {
    ...
    if (baseName.length > 6) {
      ...
      suggestions
          .add('${baseName.substring(0, 4)}${DateTime.now().year % 100}');
    }
