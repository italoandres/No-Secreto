...
class UIStateValidator {
  ...
  static ValidationResult validateRenderingState(MatchesController controller) {
    try {
      EnhancedLogger.info('Iniciando validação do estado de renderização',
          tag: 'UI_VALIDATOR');
      ...
      if (notifications.length != notificationsCount) {
        issues.add(
            'Inconsistência: notifications.length (${notifications.length}) != notificationsCount.value ($notificationsCount)');
      }
      ...
      for (int i = 0; i < notifications.length; i++) {
        ...
        if (notification['profile'] == null) {
          ...
          if (profile['displayName'] == null ||
              profile['displayName'].toString().isEmpty) {
            issues.add('Notificação $i: displayName está vazio');
          }
          if (profile['userId'] == null ||
              profile['userId'].toString().isEmpty) {
      ...
      EnhancedLogger.info('Validação concluída', tag: 'UI_VALIDATOR', data: {
        'isValid': isValid,
      ...
      if (!isValid) {
        EnhancedLogger.warning('Problemas encontrados na validação',
            tag: 'UI_VALIDATOR', data: {'issues': issues});
      }

      return result;
    } catch (e) {
      EnhancedLogger.error('Erro durante validação',
          tag: 'UI_VALIDATOR', error: e);
  ...
  static Future<void> forceSyncControllerToUI(
      MatchesController controller) async {
    try {
      EnhancedLogger.info('Iniciando sincronização forçada controller -> UI',
          tag: 'UI_VALIDATOR');
      ...
      EnhancedLogger.error('Erro durante sincronização forçada',
          tag: 'UI_VALIDATOR', error: e);
      rethrow;
    }
  }
  ...
  static Map<String, dynamic> debugUIState(MatchesController controller) {
    try {
      EnhancedLogger.info('Coletando debug completo do estado da UI',
          tag: 'UI_VALIDATOR');
      ...
      debugData['notifications_data'] = notifications
          .map((notification) => {
                'profile': {
      ...
      EnhancedLogger.error('Erro ao coletar debug do estado da UI',
          tag: 'UI_VALIDATOR', error: e);
  ...
  static Future<bool> testUIResponsiveness(MatchesController controller) async {
    try {
      ...
      EnhancedLogger.error('Erro no teste de responsividade',
          tag: 'UI_VALIDATOR', error: e);
      return false;
    }
  }
