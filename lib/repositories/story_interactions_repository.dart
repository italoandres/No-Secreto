...
class StoryInteractionsRepository {
  ...
  static Stream<List<StoryLikeModel>> getLikesStream(String storyId) {
    return _firestore
        ...
        .map((snapshot) => snapshot.docs
            .map(
                (doc) => StoryLikeModel.fromJson({...doc.data(), 'id': doc.id}))
            .toList());
  }
  ...
  static Future<String?> addComment({
    ...
    try {
      ...
      final docRef =
          await _firestore.collection('story_comments').add(comment.toJson());
  ...
  static Stream<List<StoryCommentModel>> getCommentsStream(String storyId) {
    return _firestore
        ...
        .map((snapshot) => snapshot.docs
            .map((doc) =>
                StoryCommentModel.fromJson({...doc.data(), 'id': doc.id}))
            .toList());
  }

  /// Obtém respostas de um comentário
  static Stream<List<StoryCommentModel>> getRepliesStream(
      String parentCommentId) {
    return _firestore
        .collection('story_comments')
  ...
  static Future<bool> toggleFavorite(String storyId,
      {String contexto = 'principal'}) async {
    try {
      final userId = FirebaseAuth.instance.currentUser?.uid;
      if (userId == null) {
        ContextDebug.logCriticalError(
            'toggleFavorite', 'Usuário não logado', contexto);
        return false;
      }
      ...
      if (storyId.isEmpty) {
        ContextDebug.logCriticalError(
            'toggleFavorite', 'StoryId vazio', normalizedContext);
        return false;
      }
      ...
      final favoriteRef = _firestore
          ...
          .where('userId', isEqualTo: userId)
          .where('contexto',
              isEqualTo: normalizedContext); // USAR CONTEXTO NORMALIZADO
      ...
      ContextDebug.logCriticalError(
          'toggleFavorite', 'Erro ao favoritar story: $e', contexto);
      return false;
    }
  }

  /// Verifica se o usuário favoritou o story
  static Future<bool> hasUserFavorited(String storyId,
      {String contexto = 'principal'}) async {
    try {
      final userId = FirebaseAuth.instance.currentUser?.uid;
      if (userId == null) {
        ContextDebug.logCriticalError(
            'hasUserFavorited', 'Usuário não logado', contexto);
        return false;
      }
      ...
      if (storyId.isEmpty) {
        ContextDebug.logCriticalError(
            'hasUserFavorited', 'StoryId vazio', normalizedContext);
        return false;
      }
      ...
      for (final doc in favorite.docs) {
        ...
        final docContext =
            ContextValidator.normalizeContext(data['contexto'] as String?);
      ...
      ContextDebug.logCriticalError(
          'hasUserFavorited', 'Erro ao verificar favorito: $e', contexto);
      return false;
    }
  }
  ...
  static Stream<List<String>> getUserFavoritesStream(
    ...
    if (userId == null) {
      ContextDebug.logCriticalError(
          'getUserFavoritesStream', 'Usuário não logado', contexto);
      return Stream.value([]);
    }
    ...
    if (!ContextValidator.validateAndLog(contexto, 'getUserFavoritesStream')) {
      ContextDebug.logCriticalError('getUserFavoritesStream',
          'Contexto inválido, usando contexto normalizado', normalizedContext);
    }
    ...
    return ContextDebug.measurePerformance(
        'getUserFavoritesStream_query', normalizedContext, () {
      return _firestore
          .collection('story_favorites')
          .where('userId', isEqualTo: userId)
          .where('contexto',
      ...
      ContextDebug.logLoad(normalizedContext, 'story_favorites',
          snapshot.docs.length, 'getUserFavoritesStream');
      ...
      if (invalidDocs.isNotEmpty) {
        ContextDebug.logSummary('favorites_leak_detected', normalizedContext, {
          ...
          'leakDetails': invalidDocs
              .map((doc) => {
                    'docId': doc.id,
                    'docContext':
                        (doc.data() as Map<String, dynamic>)['contexto']
                  })
              .toList()
        });
      }
      ...
      final storyIds = validDocs
          .map((doc) =>
              (doc.data() as Map<String, dynamic>)['storyId'] as String)
          .toList();

      ContextDebug.logFilter(normalizedContext, snapshot.docs.length,
          storyIds.length, 'getUserFavoritesStream');
      ContextDebug.logSummary('favorites_result', normalizedContext,
          {'totalFavorites': storyIds.length, 'storyIds': storyIds});
  ...
  static Stream<List<String>> getAllUserFavoritesStream() {
    ...
    return _firestore
      ...
      docs.sort((a, b) {
        final aTime =
            (a.data()['dataCadastro'] as Timestamp?) ?? Timestamp.now();
        final bTime =
            (b.data()['dataCadastro'] as Timestamp?) ?? Timestamp.now();
        return bTime.compareTo(aTime); // Descending
      });

      final storyIds =
          docs.map((doc) => doc.data()['storyId'] as String).toList();
      print(
          '✅ INTERACTIONS: TODOS os favoritos encontrados: ${storyIds.length} - IDs: $storyIds');
      return storyIds;
    });
  }

  /// Migra favoritos antigos que não têm contexto
  static Future<void> _migrateLegacyFavorites(
  ...
  static Future<void> _sendMentionNotifications(
      String storyId, List<String> mentions, UsuarioModel mentioner) async {
  ...
  static Future<void> shareStory(String storyId) async {
    try {
      ...
      final shareText =
          'Confira este story incrível no No Secreto com Deus Pai!\n\n$storyUrl';
