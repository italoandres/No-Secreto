...
class AcceptedMatchesRepository {
  ...
  Future<List<AcceptedMatchModel>> getAcceptedMatches(
    ...
    try {
      ...
      if (useCache && _isCacheValid(userId)) {
        debugPrint('AcceptedMatchesRepository: Usando cache para usuário $userId');
        return _cache[userId] ?? [];
      }
      
      debugPrint('AcceptedMatchesRepository: Buscando matches aceitos para usuário $userId');
      
      // Buscar notificações de interesse aceitas
      final acceptedNotifications = await _getAcceptedInterestNotifications(userId);
      ...
      for (final notification in acceptedNotifications) {
        try {
          ...
          debugPrint('AcceptedMatchesRepository: Erro ao converter notificação ${notification.id}: $e');
      ...
      debugPrint('AcceptedMatchesRepository: Encontrados ${matches.length} matches aceitos');
      return matches;
      
    } catch (e) {
      debugPrint('AcceptedMatchesRepository: Erro ao buscar matches aceitos: $e');
      
      // Retornar cache se disponível, mesmo expirado
      if (_cache.containsKey(userId)) {
        debugPrint('AcceptedMatchesRepository: Retornando cache expirado devido ao erro');
        return _cache[userId] ?? [];
      }
    ...
    return _getAcceptedInterestNotificationsStream(userId)
      ...
      for (final notification in notifications) {
        try {
          ...
          debugPrint('AcceptedMatchesRepository: Erro no stream para notificação ${notification.id}: $e');
        }
      }
  ...
  Future<AcceptedMatchModel?> getMatchById(String userId, String matchId) async {
    try {
      // Verificar cache primeiro
      final cachedMatches = _cache[userId];
      if (cachedMatches != null) {
        final cachedMatch = cachedMatches.where((m) => m.notificationId == matchId).firstOrNull;
        if (cachedMatch != null) {
          return cachedMatch;
        }
      }
      ...
      debugPrint('AcceptedMatchesRepository: Erro ao buscar match $matchId: $e');
      return null;
    }
  }
  ...
  Future<bool> updateReadStatus(String matchId, bool isRead) async {
    try {
      ...
      debugPrint('AcceptedMatchesRepository: Erro ao atualizar status de leitura: $e');
      return false;
    }
  }
  ...
  Future<int> getUnreadMatchesCount(String userId) async {
    try {
      ...
      debugPrint('AcceptedMatchesRepository: Erro ao contar matches não lidos: $e');
      return 0;
    }
  }
  
  /// Busca notificações de interesse aceitas do Firebase
  Future<List<InterestNotificationModel>> _getAcceptedInterestNotifications(String userId) async {
    final query = await _firestore
        .collection('interest_notifications')
        .where('toUserId', isEqualTo: userId)
        .where('status', isEqualTo: 'accepted')
  ...
  Stream<List<InterestNotificationModel>> _getAcceptedInterestNotificationsStream(String userId) {
    return _firestore
        .collection('interest_notifications')
        .where('toUserId', isEqualTo: userId)
        .where('status', isEqualTo: 'accepted')
  ...
  Future<AcceptedMatchModel?> _convertNotificationToMatch(
    ...
    try {
      ...
      if (!otherUserDoc.exists) {
        debugPrint('AcceptedMatchesRepository: Usuário $otherUserId não encontrado');
        return null;
      }
      ...
      debugPrint('AcceptedMatchesRepository: Erro ao converter notificação: $e');
      return null;
    }
  }
