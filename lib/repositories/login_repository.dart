...
class LoginRepository {
  ...
  static Future<void> _navigateAfterAuth() async {
    ...
    try {
      ...
      if (!welcomeShown) {
        ...
        debugPrint('LoginRepository: Welcome já visto, indo direto para HomeView');
    ...
    try {
      ...
      if (firestoreSexoString != null && firestoreSexoString != 'none') {
        ...
        if (TokenUsuario().sexo != firestoreSexo) {
          debugPrint('🔄 Atualizando TokenUsuario: ${TokenUsuario().sexo.name} → ${firestoreSexo.name}');
          TokenUsuario().sexo = firestoreSexo;
          debugPrint('✅ TokenUsuario atualizado com valor do Firestore: ${firestoreSexo.name}');
        } else {
          debugPrint('✅ Sexo já está sincronizado: ${firestoreSexo.name}');
        }
      } else {
        debugPrint('⚠️ Sexo não definido no Firestore - usuário pode precisar completar perfil');
      }
    } catch (e) {
  ...
  static Future<void> loginComGoogle() async {
    ...
    try {
      ...
      if(e.code == 'user-disabled') {
        ...
        Get.rawSnackbar(message: AppLanguage.lang('credenciais_invalidas') ?? 'Credenciais inválidas');
      } else {
  ...
  static loginComApple() async {
    ...
    if(kIsWeb || !Platform.isIOS) {
      Get.rawSnackbar(message: 'Opção temporariamente indisponível para aparelhos Android!');
      return;
    }
  ...
  static Future<void> login({
    ...
    try {
      ...
      if(e.code == 'user-not-found') {
        ...
        Get.rawSnackbar(message: '🌐 Erro de conexão. Verifique sua internet e tente novamente.');
      } else if(e.code == 'unknown' && e.message?.contains('Connection reset by peer') == true) {
        Get.rawSnackbar(message: '🔄 Problema de conexão. Tente novamente em alguns segundos.');
      } else {
