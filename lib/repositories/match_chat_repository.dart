...
class MatchChatRepository {
  ...
  static Future<void> updateChatFields(
      String chatId, Map<String, dynamic> fields) async {
    try {
      print('📝 Atualizando campos do chat $chatId: ${fields.keys.join(', ')}');

      await _firestore.collection(_chatsCollection).doc(chatId).update(fields);
  ...
  static Future<void> _incrementUnreadCount(
    ...
    try {
      ...
      print(
          '📊 Contador não lidas incrementado: $recipientId = ${currentCount + 1}');
    } catch (e) {
  ...
  static Future<Map<String, dynamic>> getChatStats(String chatId) async {
    try {
      ...
      final totalUnread =
          chat.unreadCount.values.fold<int>(0, (sum, count) => sum + count);
