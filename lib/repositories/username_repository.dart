...
enum UsernameAvailability { available, unavailable, checking, error }
...
class UsernameRepository {
    ...
    try {
      ...
      await _firestore.runTransaction((transaction) async {
        ...
        DocumentReference usernameRef =
            _firestore.collection('usernames').doc(cleanUsername);
        ...
        if (oldUsername != null && oldUsername.isNotEmpty) {
          String oldCleanUsername =
              oldUsername.replaceAll('@', '').toLowerCase().trim();
          if (oldCleanUsername != cleanUsername) {
            DocumentReference oldUsernameRef =
                _firestore.collection('usernames').doc(oldCleanUsername);
      ...
      print(
          'DEBUG USERNAME: Username $cleanUsername salvo para usuário $userId');
    } catch (e) {
  ...
  static Future<bool> canUserUseUsername(String username) async {
    try {
      ...
      print(
          'DEBUG USERNAME: Erro ao verificar se usuário pode usar username: $e');
      return false;
    }
  }
  ...
  static Future<void> cleanupOrphanedUsernames() async {
    try {
      QuerySnapshot usernamesSnapshot =
